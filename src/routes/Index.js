  ...
  render() {
    const menu = (
      <Menu>
        ...
        <Menu.Item key="3">重新载入</Menu.Item>
      </Menu>
    );
      ...
      <Layout className={styles.tablePage}>
        ...
        <Content style={{ overflowY: 'auto', padding: '0 10px 0 16px' }}>
          ...
          <Table
            ...
            dataSource={this.props.pagedata.rows}
            size="middle"
            pagination={false}
            rowSelection={{
              type: 'checkbox',
          ...
          <div className={styles.tablePagination}>
            <Pagination pageSize={this.props.pagedata.page.size || 20} showSizeChanger showQuickJumper defaultCurrent={this.props.pagedata.page.index} total={this.props.pagedata.page.total} showTotal={(total, range) => `${range[0]}-${range[1]} of ${total} items`} />
          </div>
        </Content>
      </Layout>
    );
